...
;tf.img_path = '../others/plugin/theme_tyrano_01_1280/image/config/';
	...
	tf.config_x       = [1104, 456, 512, 568, 624, 680, 736, 792, 848, 904, 960]; // X座標（共通）

	tf.config_y_bgm   = 168; // BGMのY座標
	tf.config_y_se    = 248; // SEのY座標
	tf.config_y_ch    = 328; // テキスト速度のY座標
...
;	画面右上の「Back」ボタン
	[button fix="true" graphic="&tf.img_path + 'c_btn_back.png'" enterimg="&tf.img_path + 'c_btn_back2.png'" target="*backtitle" x="1152" y="592"]
...
;	未読スキップ-OFF
	[button name="unread_off" fix="true" target="*skip_off" graphic="&tf.btn_path_off" width="160" height="48" x="456" y="488"]

;	未読スキップ-ON
	[button name="unread_on"  fix="true" target="*skip_on"  graphic="&tf.btn_path_off" width="160" height="48" x="624" y="488"]
